...
import type { FontSizePreset, SilenceDurationMs } from "@/types/settings";
  ...
  return (
    <div className="flex flex-col gap-8 p-4 pb-12">
      <section className="flex flex-col gap-3">
        <SectionHeading>字幕サイズ</SectionHeading>
      ...
      <section className="flex flex-col gap-3">
        <SectionHeading>無音確定時間</SectionHeading>
        <div className="flex flex-wrap gap-2">
      ...
      <section className="flex flex-col gap-3">
        <SectionHeading>自動言語判定の初期値</SectionHeading>
        <Switch
        ...
        >
          <Switch.Content>
            ...
            <span className="text-[length:var(--text-sm)]">起動時に自動判定を有効にする</span>
          </Switch.Content>
        </Switch>
      </section>

      <section className="flex flex-col gap-2 rounded-[var(--radius-card)] bg-[var(--color-paper-2)] p-4">
        <SectionHeading>モックモード</SectionHeading>
      ...
      <section className="flex flex-col gap-3">
        <SectionHeading>履歴の全削除</SectionHeading>
        <p className="text-[length:var(--text-sm)] text-[var(--color-ink-2)]">
      ...
      <section className="flex flex-col gap-1 text-[length:var(--text-xs)] text-[var(--color-muted)]">
        ...
        <span>音声は保存されません。自動言語判定は補助機能です。</span>
      </section>
    </div>
  );
}
